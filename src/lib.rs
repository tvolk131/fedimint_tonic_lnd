    ...
    MP: AsRef<Path> + Into<PathBuf> + std::fmt::Debug,
{
    ...
    let client = Client {
        ...
        #[cfg(feature = "staterpc")]
        state: staterpc::state_client::StateClient::with_interceptor(
            channel.clone(),
...
mod tls {
    ...
    pub(crate) async fn config(
        ...
        let cert = Certificate::from_pem(std::fs::read(path.as_ref().to_str().unwrap()).unwrap());
